...
# 검증 상태 요약 (Phase 1 ~ Phase 6, TASK-004 ~ TASK-022)
...
## 5. Service Layer & External Integration Boundary (TASK-022)
...
- Concurrency 정책:
  ...
  - Fail-Fast 정책 동작 확인
- Admin Query는 Service Layer를 우회하여 Query Service 경로로 분리됨 확인
- 외부 MQ 미도입, In-Process 동기 이벤트 처리 범위 유지 확인
- Phase 5 계약(Freeze / Snapshot / State Contract) 훼손 없음 확인

---

## 6. 검증 방법 및 기준

- 모든 TASK는 `scripts/verify_task_xxx.py` 스크립트를 통해 계약 동작 검증을 수행함
- 검증 스크립트는 계약 위반 여부 확인을 위한 보조 도구로 사용되며, 정책 정의 자체를 대체하지 않음
- 구조 계약(계층 분리, DTO 분리, 상태 변경 경로 제한)은 코드 리뷰 및 설계 검증을 통해 추가 확인함
...
- 상태: **Phase 6 진행 중**  
  ...
  - Phase 6 진행:
    - Service Layer 구축 완료 (TASK-022)
      ...
      - Admin Query는 Query Service 경로로 분리
      - Phase 5 계약 훼손 없음 확인
...
- 인터뷰 세션 실행을 외부에서 호출 가능한 **API 인터페이스 계층 구현 (TASK-023 예정)**
- LLM / RAG 연동 계층 설계
  - 질문 생성 전략
  - Follow-up 전략
  - 직무 기반 질문 선택
...
- 서비스 오케스트레이션(Service Layer + API Layer) 구조 확정
...
- Phase 5에서 확정된 통합 아키텍처를 외부 API 및 서비스 인터페이스로 안전하게 노출
- Command / Query 분리 구조를 유지한 채 외부 호출 경계 확정
