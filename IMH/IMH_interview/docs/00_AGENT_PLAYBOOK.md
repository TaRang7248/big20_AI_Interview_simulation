# 00_AGENT_PLAYBOOK — Phase 5 기준 통합 반영본 (2026-02-11)
...
## 0. Phase 5 기준 필수 반영 항목 (Session 통합 대응)
...
- `_refs/` 폴더의 문서는 **설계 스펙 원본 문서**이다.
  - 에이전트는 `_refs` 문서를 직접 구현 기준으로 사용하지 않는다.
  - `_refs`의 내용은 반드시 docs 문서에 반영된 이후에만 구현 기준이 된다.
...
1) **인터뷰 동작 정책(최상위 계약)**
   ...
   - 핵심: 실전/연습 모드 정책 차이, 세션 생성 시 정책 고정(freeze), 질문 전환 조건(완료/침묵/시간), 침묵 처리(답변 후/무응답), 최소 질문 수 보장, 조기 종료 조건, 결과 공개 정책(시점/범위/2주 내 통지 보장)
...
## 5. Phase 5 기준 통합 로드맵 (Session Engine 중심)

현재 프로젝트는 Phase 0~4가 완료된 상태이며,
Phase 5 후반부(Session Engine 통합 설계 단계)에 진입하였다.

따라서 신규 개발 우선순위는 다음 기준을 따른다.

### [Step 1] Interview Session Engine 설계
- 공고 기반 정책 freeze 구조 설계
- 질문 전환/침묵 처리/최소 질문 수 보장 로직 정의
- Redis 상태 관리 + PostgreSQL 결과 확정 분리
...
- 질문 단위 평가
- 세션 종료 후 집계
- evidence_data JSON 계약 준수

### [Step 4] 정책 불변성 보장
- 세션 생성 시점 기준 정책 고정
- 진행 중 공고 변경은 세션에 영향 없음
- 과거 세션 재계산 금지
...
## 8. Phase 5(Session Engine) 설계 전 필수 점검 항목
...
Session Engine 설계 시 반드시 준수할 정책 규칙:

- 세션은 생성 시점의 공고 정책을 기준으로 고정(freeze)된다.
- 세션 시작 이후 공고 설정 변경은 진행 중 세션에 영향을 주지 않는다.
- 최소 질문 수 충족 이전에는 점수 기반 조기 종료를 허용하지 않는다.
- 침묵은 “답변 후 침묵”과 “무응답 침묵”을 구분하여 처리한다.
- 무응답 발생 시 무응답 플래그를 평가 엔진에 전달한다.
- 질문 수행 완료 기준은 다음 중 하나로 고정한다:
  1) 답변 완료 버튼 클릭
  2) 침묵 자동완료 시간 초과
  3) 질문 제한시간 초과
...
### 참고 문서(필수)
- 인터뷰 정책 스펙: `26.02.11(수)인터뷰 정책 스펙.md`
