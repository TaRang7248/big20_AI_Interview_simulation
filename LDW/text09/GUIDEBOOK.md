...
### 4.1 STT 엔진 우선순위 및 로직 최적화
- **Google Web Speech 주력 설정**: 실시간 인식 효율을 높이기 위해 Google Web Speech API를 제1 엔진으로 사용합니다.
- **Whisper 보조 엔진 도입**: Google 엔진이 결과물을 반환하지 못하거나 오류가 발생할 경우에만 로컬 Whisper 모델이 보조적으로 작동하도록 로직을 개선했습니다. 이를 통해 시스템 리소스를 절약하면서도 인식 안정성을 확보했습니다.
- **초고속 분석 파이프라인**: 병렬 실행 방식 대신 순차 실행(Fallback) 방식으로 변경하여 불필요한 연산 부하를 줄였습니다.
...
- **STT 엔진 우선순위 변경**: Google Web Speech를 주 엔진으로, Whisper를 보조 엔진으로 설정하여 속도와 정확도를 모두 잡았습니다.
