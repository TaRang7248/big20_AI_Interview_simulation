...
## 4. 주요 업데이트 내용 (2026-02-26)

### 4.1 STT 엔진 우선순위 및 로직 최적화 (2026-02-26 업데이트)
...
### 4.2 프로젝트 구조 최적화 및 슬림화
- **루트 디렉토리 정리**: `app`, `data`, `db`, `models`, `scripts`, `static`, `uploads`, `server.py` 등 필수 항목 외의 부수적인 파일들을 관련 폴더로 이동하여 깔끔한 구조를 유지합니다.
- **`server.py` 전용화**: 서버 실행과 브라우저 자동 호출 기능에만 집중하도록 코드를 최적화하고, 내부의 모든 주석을 한국어로 전면 개편했습니다.

### 4.3 의존성 고정 및 안정화
- **패키지 버전 고정**: `numpy==1.26.2` 및 `pyannote-audio==3.1.1` 버전을 명시적으로 고정하여 설치 시 발생할 수 있는 호환성 문제를 차단했습니다.

## 5. 이전 업데이트 내역 (2026-02-25)
...
### 4.2 서버 스크립트 최적화 및 비동기 파이프라인 도입
- **평가 연산 비동기 처리**: 사용자의 답변 제출 시 이전까지는 LLM(Gemini)이 (1) 답변 평가 (2) 다음 질문 생성을 동시에 수행하여 대기 시간이 길어졌습니다. 업데이트를 통해 **다음 질문 생성을 우선적으로 수행**하여 프론트엔드로 즉시 반환하고, 비교적 시간이 오래 걸리는 **답변 평가는 백그라운드 태스크(Background Task)로 분리**하여 비동기적으로 데이터베이스에 저장되도록 파이프라인 구조를 개선했습니다. 이로써 사용자 체감 대기 속도가 대폭 단축되었습니다.
- **`server.py` 정리**: `server.py`는 서버 구동 및 웹 브라우저 자동 실행이라는 본연의 기능에 충실하도록 코드를 정리하고 모든 안내 메시지와 코드 주석을 한국어로 표준화하였습니다.
...
### 4.4 프로젝트 구조 정리 및 최적화 (2026-02-25 추가)
- **루트 디렉토리 정리**: 지정된 주요 폴더(`app`, `data`, `db`, `models` 등) 및 필수 파일 외의 항목들을 정리하여 관리 효율성을 높였습니다.
- **`server.py` 슬림화**: 서버 실행과 브라우저 자동 호출 기능에만 집중하도록 코드를 최적화하고, 모든 내부 주석을 한국어로 전면 개편했습니다.
- **의존성 고정**: `numpy==1.26.2` 및 `pyannote-audio==3.1.1` 버전을 명시적으로 고정하여 설치 시 발생할 수 있는 호환성 문제를 사전에 차단했습니다.
