...
## 2. 주요 업데이트 내용 (2026-02-27 업데이트)

### 2.1 STT 엔진 우선순위 및 로직 최적화
- **Google Web Speech 주력 설정**: 실시간 인식 효율을 높이기 위해 Google Web Speech API를 제1 엔진으로 사용합니다.
- **음성 미인식 처리 개선**: Google Web Speech에서 음성을 인식하지 못한 경우(`UnknownValueError`) 즉시 '답변 없음'으로 처리하여 불필요한 보조 엔진 전환을 방지합니다.
- **Whisper 보조 엔진 안정화**: Google Web Speech 서비스에 접속할 수 없는 오류(`RequestError`) 발생 시에만 로컬 Whisper 모델이 보조적으로 작동하도록 로직을 개선하여 리소스 낭비를 줄이고 시스템 가동률을 높였습니다.

### 2.2 코드베이스 주석 및 로깅 전면 한글화
- 백엔드 라우터(`app/routers/`), 서비스(`app/services/`), 핵심 설정 파일(`main.py`, `database.py` 등) 및 프론트엔드(`static/app.js`, `index.html`, `styles.css`)의 주요 영문 주석 및 로깅 메시지, 리턴 구문을 모두 한국어로 번역하여 시스템 가독성과 유지보수성을 극대화했습니다.

### 2.3 프로젝트 구조 최적화 및 디렉토리 단일화
- **루트 디렉토리 정리**: `app`, `static`, `models`, `server.py`, `requirements.txt` 등 핵심 구성 요소를 제외한 잉여 스크립트 파일 및 더미 항목들을 `data/archive/` 폴더 내부로 일괄 이동하여 프로젝트 최상단 구조를 매우 깔끔하게 정리했습니다.
- **`server.py` 전용화**: 서버 실행과 브라우저 자동 호출 기능에만 집중하도록 코드를 최적화하고 불필요한 로직을 제거했습니다.

### 2.4 의존성 고정 및 안정화
- **패키지 버전 고정**: `requirements.txt`에 `numpy==1.26.2` 및 `pyannote-audio==3.1.1` 버전을 강제 고정 명시하여 라이브러리 설치 시 발생하는 버전 충돌 및 호환성 문제를 완연히 해결했습니다.

### 2.5 회원 탈퇴 기능 추가 (New)
- **내 정보 수정 내 탈퇴 버튼 생성**: 지원자와 관리자 모두 '내 정보 수정' 페이지에서 '비밀번호 변경' 버튼 바로 오른쪽에 동일한 디자인의 **'회원 탈퇴'** 버튼을 사용할 수 있습니다.
- **보안 및 확인 절차**: 버튼 클릭 시 브라우저 프롬프트를 통해 현재 비밀번호를 입력받아 확인하며, 이후 "정말 회원을 탈퇴하시겠습니까?"라는 최종 확인 메시지를 통해 실수로 인한 탈퇴를 방지합니다.
- **완전한 데이터 삭제**: 탈퇴 수락 시 데이터베이스에서 해당 회원의 모든 정보가 즉시 삭제(DELETE)되며, 자동으로 로그아웃되어 초기 로그인 화면으로 이동합니다.
- **취소 기능**: 비밀번호 입력 단계나 최종 확인 단계에서 취소할 경우 기존 메뉴로 안전하게 돌아옵니다.

### 2.6 로그인 화면 피드백 개선 (New)
- **상세 오류 메시지 표시**: 로그인 실패 시 단순히 "서버 연결 실패"만 표시하던 기존 방식을 개선하여, 서버(FastAPI)에서 전달하는 상세 에러 메시지(`detail`)를 화면상의 Toast 알림으로 즉시 표시합니다.
- **예외 처리 강화**: 존재하지 않는 아이디 또는 틀린 비밀번호 입력 시 각각의 상황에 맞는 한국어 안내 메시지가 나타나 사용자가 오류 원인을 쉽게 파악할 수 있습니다.

### 2.7 데이터 백업 및 복원 검증 및 스크립트 한글화 (New)
- **데이터 이관 검증**: `scripts/export_db.py` 및 `scripts/import_db.py` 작동을 분석하여 `JSON` 형태의 데이터베이스 백업 시 누락이나 중복(`ON CONFLICT DO NOTHING`) 없이 성공적으로 데이터 이관이 이뤄짐을 검증했습니다.
- **스크립트 주석 한글화**: 백업 및 복구 스크립트 내부의 모든 실행 코멘트와 안내 메시지(print)를 올바른 한글로 명확하게 번역하여 유지보수성을 향상시켰습니다.
...
- **Google Web Speech API**: 실시간 음성 인식을 위한 주력 STT 엔진 (SpeechRecognition 라이브러리 활용) **[UPDATED]**
- **OpenAI Whisper (Local)**: 음성 인식 (STT) 보조 및 폴백 엔진 **[UPDATED]**
- **Google Gemini 2.0 Flash**: 면접 질문 생성, 답변 분석, 평가(LLM) 통합 모델
...
## 7. 상세 파일 구조 설명 (File Structure)

본 프로젝트는 관리 효율성을 위해 계층화된 디렉토리 구조를 따릅니다.

### [루트 디렉토리 파일]
- **`server.py`**: 서비스의 메인 엔트리 포인트입니다. FastAPI 서버(uvicorn)를 구동함과 동시에 사용자의 기본 웹 브라우저를 자동으로 열어줍니다. 서버 실행과 브라우저 호출 기능에 집중하여 최적화되어 있으며, 모든 주석이 한국어로 제공됩니다.
- **`requirements.txt`**: 프로젝트 실행을 위한 패키지 의존성 목록입니다. `numpy==1.26.2`, `pyannote-audio==3.1.1`로 버전이 엄격히 고정되어 안정성을 확보했습니다.
...
- **비디오 심층 분석 통합**: `app/routers/video_router.py` 엔드포인트를 통해 실시간으로 전송되는 프레임을 수신하며, 면접 종료 시 자동으로 `.webm` 영상 파일을 스캔하여 **OpenCV**, **MoveNet Thunder**, **DeepFace**로 정밀 분석합니다. 실시간 프레임 분석 데이터 로그(`video_logs`)와 최종 처리 결과를 융합하여 태도/인성 평가에 반영합니다.
...
- **STT 엔진 우선순위 최적화**: Google Web Speech를 주 엔진으로 사용하며, 음성 미인식 시 즉시 '답변 없음'을 표기합니다. 서비스 접속 불가 시에만 Whisper로 전환되는 폴백 시스템을 구축했습니다.
...
### 신규 업데이트 (관리자 지원자 기능 개선) **[NEW]**
- **지원자 정보 삭제 기능 추가**: 관리자 대시보드의 '지원자 현황' 테이블 내 '질문/답변 확인' 란에서 상세보기 옆에 [삭제] 버튼이 새롭게 추가되었습니다. 버튼 클릭 시 해당 지원자의 면접 진행 기록(Interview_Progress) 및 면접 결과(Interview_Result) 등을 DB에서 완전 삭제하여 데이터를 관리할 수 있습니다.
...
## 10. Wav2Lip 기반 립싱크 비디오 연동 (업데이트 노트)
