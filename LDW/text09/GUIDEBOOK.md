...
- **서버 실행 스크립트 최적화 (2026-02-25)**: `server.py`를 서버 구동 및 자동 웹 브라우저 실행 기능만 전용으로 개편하고 모든 주석을 한국어로 변경했습니다.
- **프로젝트 구조 정량화 및 서비스 주석 한글화 (2026-02-25)**: 루트 디렉토리를 정리하여 관리 효율성을 높였으며, 모든 비즈니스 로직 서비스(`app/services/*.py`)의 주석을 한국어로 전면 수정했습니다.
