...
"""
        ...
        if not content:
            ...
            return {"score": 0, "feedback": "AI 응답 오류", "is_follow_up": False, "pass_fail": "보류"}
            
        try:
            parsed = json.loads(content)
            if parsed is None:
                return {"score": 0, "feedback": "AI 응답 오류 (Null)", "is_follow_up": False, "pass_fail": "보류"}
            return parsed
        except Exception as e:
            print(f"LLM JSON Error: {e}")
            return {"score": 0, "feedback": "AI 응답 형식 오류", "is_follow_up": False, "pass_fail": "불합격"}
    async def evaluate_architecture(self, image_data_url):
        """Evaluates an architecture diagram image using GPT-4o Vision."""
        prompt = """
이 이미지는 지원자가 면접 중에 그린 시스템 아키텍처 다이어그램입니다.
이 아키텍처의 타당성, 확장성, 그리고 보안성을 전문적인 시각에서 평가해주세요.
다음 형식으로 피드백을 제공해 주세요 (JSON):
{
    "score": 80,
    "feedback": "3-Tier 구조가 잘 표현되었으나, DB 이중화가 누락되어 있습니다."
}
"""
        response = self.client.chat.completions.create(
            model=self.model,
        ...
        try:
            parsed = json.loads(content)
            print(f"DEBUG: RAW LLM CONTENT: {content}")
            if parsed is None:
            ...
            return {"score": 0, "feedback": "AI 응답 형식이 올바르지 않습니다.", "is_follow_up": False, "pass_fail": "보류"}
