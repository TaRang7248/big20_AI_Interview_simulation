...
class LLMService:
    ...
    async def generate_initial_question(self, job_title, vector_search_func=None):
        # Optional: Use RAG to find relevant initial questions
        relevant_context = ""
        if vector_search_func:
            similar = await vector_search_func(job_title, limit=3)
            if similar:
                relevant_context = "\n참고할 수 있는 기출 질문들:\n" + "\n".join([s.question for s in similar])

        prompt = f"""
        당신은 전문 면접관입니다. 지원자가 지원한 직무는 '{job_title}'입니다.
        {relevant_context}
        
        지원자의 직무와 위 기출 질문들을 참고하여 적합한 첫 번째 면접 질문을 하나 생성해 주세요.
        질문은 정중한 말투의 한국어로 작성해 주세요.
        질문만 반환해 주세요.
        """
        response = self.client.chat.completions.create(
            model=self.model,
            messages=[{"role": "system", "content": "You are a professional interviewer expert in recruitment."},
