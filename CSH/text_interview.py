...
class InterviewReportGenerator:
                ...
                for keyword in keywords:
                    if keyword in answer_lower:
                        ...
                        if context not in star_analysis[star_element]["examples"]:
                            star_analysis[star_element]["examples"].append(
                                f"...{context}..."
                            )
                        break  # 하나의 키워드만 카운트
    ...
    def generate_star_feedback(self, star_analysis: dict) -> str:
        ...
        if strong_elements:
            feedback.append(
                f"✅ 강점: {', '.join(strong_elements)} 요소가 잘 포함되어 있습니다."
            )

        if weak_elements:
            feedback.append(
                f"📝 개선 필요: {', '.join(weak_elements)} 요소를 더 보완하면 좋겠습니다."
            )
...
위 기준에 따라 지원자의 면접 답변을 평가하고, 각 항목별로 1~5점 척도로 점수를 매겨주세요. 평가한 결과를 바탕으로 합격 혹은 불합격 여부도 판단해주세요."""
    ...
    def generate_report(self, chat_history: list, video_metrics: dict = None) -> str:
        ...
        if keywords["tech_keywords"]:
            ...
            tech_str = ", ".join(
                [f"{kw}({cnt}회)" for kw, cnt in keywords["tech_keywords"][:5]]
            )
        ...
        if keywords["general_keywords"]:
            report.append("  • 주요 표현:")
            general_str = ", ".join(
                [f"{kw}({cnt}회)" for kw, cnt in keywords["general_keywords"][:8]]
            )
        ...
        if video_metrics:
            ...
            report.append(
                f"  • 발음 명확성: {video_metrics.get('pronunciation_clarity', 'N/A')}"
            )
            report.append(f"  • 시선 처리: {video_metrics.get('eye_contact', 'N/A')}")
            report.append(
                f"  • 표정 분석: {video_metrics.get('facial_expression', 'N/A')}"
            )
        else:
            report.append("  ℹ️ 텍스트 기반 면접으로 비언어적 분석이 제공되지 않습니다.")
            report.append(
                "  💡 비디오 면접 모드에서 발화 속도, 발음 명확성, 시선 처리 분석이 가능합니다."
            )
        ...
        if star_analysis["action"]["count"] < 2:
            suggestions.append(
                "• 본인이 직접 수행한 '행동'을 더 구체적으로 설명해보세요."
            )
...
def main():  # 프로그램의 메인 로직을 담는 함수
    ...
    if os.path.exists(resume_path):
        ...
        do_index = (
            input("이력서를 DB에 새로 인덱싱하시겠습니까? (y/n, default: n): ")
            .strip()
            .lower()
        )
        if do_index == "y":
        ...
        print(
            f"Warning: '{resume_path}' 파일을 찾을 수 없습니다. RAG 기능이 제한될 수 있습니다."
        )
    ...
    try:
        ...
        print(
            f"✅ LLM 모델 로드 완료: {DEFAULT_LLM_MODEL} (context window: {DEFAULT_LLM_NUM_CTX})"
        )
    except Exception as e:
    ...
    while True:  # 무한 루프
        try:
            ...
            if user_input.lower().strip() in ["exit", "종료", "quit"]:
                ...
                generate_report = (
                    input(
                        "\n📊 면접 결과 리포트를 생성하시겠습니까? (y/n, default: y): "
                    )
                    .strip()
                    .lower()
                )
                if generate_report != "n":
                    ...
                    save_report = (
                        input(
                            "\n💾 리포트를 파일로 저장하시겠습니까? (y/n, default: n): "
                        )
                        .strip()
                        .lower()
                    )
            ...
            if context_text:
                context_message = SystemMessage(
                    content=f"--- [RAG System] 참고용 이력서 관련 내용 ---\n{context_text}\n------------------------------------------"
                )
            ...
            try:
                generate_report = (
                    input(
                        "\n📊 면접 결과 리포트를 생성하시겠습니까? (y/n, default: n): "
                    )
                    .strip()
                    .lower()
                )
