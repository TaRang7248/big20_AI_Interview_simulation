...
| 순위 | 항목 | 위험도 | 조치 내용 |
|------|------|--------|-----------|
| **1** | Rate Limiting | 🟠 중간 | 로그인/회원가입 API에 요청 횟수 제한 추가 (brute-force 방지) |
| **2** | TLS 활성화 | 🟠 중간 | 운영 환경에서 HTTPS 적용 (Let's Encrypt 등) |
| **3** | 코드 실행 격리 | 🟠 중간 | Docker 컨테이너 기반 샌드박스 전환 검토 |
| **4** | 소셜 로그인 빈 패스워드 | 🟡 낮음 | 소셜 가입 시 `password_hash`를 랜덤 해시로 설정 |
